...
import { isSubagent, summaryOf } from "./notify-policy.js";
 ...
 */
function systemNotify(title, body, openUrl, sound) {
    switch (PLATFORM) {
        case 'darwin':
...
export function applySystemNotify(ctx, configOf, baseUrl = 'http://127.0.0.1:3080') {
    const sessionOpenUrl = (sessionId) => `${typeof baseUrl === 'function' ? baseUrl() : baseUrl}/?session=${encodeURIComponent(sessionId)}`;
    ...
    ctx.on('agent/status', (payload) => {
        safe(() => {
            ...
            if (!cfg.enabled || !cfg.turn)
                return;
            const now = Date.now();
            // 顺带清理早已过期的旧条目，防止长期运行后无界增长。
            if (lastTurnAt.size >= 64) {
                for (const [id, at] of lastTurnAt) {
                    if (now - at >= TURN_DEDUP_MS)
    ...
    ctx.on('agent/error', (payload) => {
        safe(() => {
            ...
            systemNotify('Agent 出错', summaryOf(detail, 80), sessionOpenUrl(payload.agent.id), cfg.sound);
        });
    }, { global: true });
