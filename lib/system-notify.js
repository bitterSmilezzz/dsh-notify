...
import { isSubagent, NOTIFY_EVENTS, probeOfficialNotify, pruneExpired, summaryOf } from "./notify-policy.js";
 ...
 */
function spawnNotifierSilicon(args, onExecError) {
    try {
...
function spawnNotifierIntel(args, onExecError) {
    try {
...
function notifyMac(title, body, openUrl, sound) {
 ...
 */
export function applySystemNotify(ctx, configOf, baseUrl = 'http://127.0.0.1:3080', probeServices, onProbeChange) {
    // 防重叠探测：惰性 + 状态缓存。lastProbe 记录最近结果，变化时回抛一次。
    // onProbeChange 回调自带 try/catch：观察方异常不得冒泡（probeNow 早于
    // safe 定义被首次调用，不能依赖后者）。
    let lastProbe = { official: false };
    const probeNow = () => {
        const state = probeOfficialNotify(ctx, probeServices);
    ...
    ctx.on(NOTIFY_EVENTS.turnDone, (payload) => {
        safe(() => {
            ...
            systemNotify('轮次完成', '该会话已结束一轮，可以切回查看', sessionOpenUrl(payload.agent.id), cfg.sound);
        });
    }, { global: true });
        ...
        try {
            ...
            if (!isSubagent(req.agent) && cfg.enabled && cfg.approval) {
                ...
                const detail = req.reason !== undefined && req.reason !== ''
                    ? `${toolName} · ${req.reason}`
                    : toolName;
                systemNotify('需要审批', summaryOf(detail, NOTIFY_BODY_MAX), sessionOpenUrl(req.agent.id), cfg.sound);
            }
        }
    ...
    ctx.on(NOTIFY_EVENTS.error, (payload) => {
        safe(() => {
            ...
            systemNotify('Agent 出错', summaryOf(detail, NOTIFY_BODY_MAX), sessionOpenUrl(payload.agent.id), cfg.sound);
        });
    }, { global: true });
    ...
    ctx.on(NOTIFY_EVENTS.sessionDone, (payload) => {
        safe(() => {
            ...
            if (!cfg.enabled || !cfg.sessionDone)
                return;
            systemNotify('会话完成', '该会话已完成，可以切回查看', sessionOpenUrl(payload.agent.id), cfg.sound);
        });
    }, { global: true });
}
